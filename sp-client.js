...
window.getLastFrontedDateTime = function (elementId, memberId) {
    ...
    const render = (isLive, endTime) => {
        ...
        if (isLive) {
            ...
            const update = () => {
                el.innerHTML = `<span class="p"><strong>Last Fronted: </strong>${timeSince(new Date(endTime))} ago</span>`;
            };
            update();
    ...
    // ✅ FIXED: normalize single-object or array into array
    on('fronthistory', (msg) => {
        if (msg.memberId !== memberId) return;

        lastData = Array.isArray(msg.data) ? msg.data : [msg.data];
